...
class TrainerBase:
    ...
    def build_dataloader(self):
        ...
        self.master_datasets = {'val':{}}
        self.master_dataloaders = {'val':{}}
        if self.rank == 0:
            datasets['expr'] = create_dataset(self.configs.data.get('train', dict))
        if hasattr(self.configs.data, 'val') and self.rank == 0:
            if self.configs.data.val.type == 'tempoSpatial_master_3d':
                #######################
                datasets['val'] = None
                self.dataset_type_li = ['tempoSpatial_cardiac_bowl_3d', 'tempoSpatial_SurgT']
                opt_config = self.configs.data.val.get('params', dict)
                for dataset_type in self.dataset_type_li:
                    if not(dataset_type in opt_config): continue
                    
                    new_opt = {'type': dataset_type, 'params':{}}
                    for key in opt_config:
                        if key in self.dataset_type_li: continue
                        new_opt['params'][key] = opt_config[key]
                    new_opt['params']['txt_file_path'] = opt_config[dataset_type]['txt_file_path']
                    self.master_datasets['val'][dataset_type] = create_dataset(new_opt)
                #######################
            else:
                datasets['val'] = create_dataset(self.configs.data.get('val', dict))
        if self.rank == 0:
            for phase in datasets.keys():
                if phase == 'val' and self.configs.data.val.type == 'tempoSpatial_master_3d':
                    for dataset_type in self.dataset_type_li:
                        length = len(self.master_datasets[phase][dataset_type])
                        self.logger.info('Number of images in {:s} data set, type is {:s}: {:d}'.format(phase, dataset_type, length))
                else:
        ...
        if self.num_gpus > 1:
            ...
            sampler = None
        dataloaders = {'train': iter(udata.DataLoader(
                        datasets['train'],
        ...
        if self.rank == 0:
            dataloaders['expr'] = udata.DataLoader(datasets['expr'],
                                                  batch_size=self.configs.experiment_evaluate.batch,
                                                  shuffle=False,
                                                  drop_last=False,
                                                  num_workers=self.configs.experiment_evaluate.num_workers,
                                                  prefetch_factor=self.configs.experiment_evaluate.get('prefetch_factor', None),
                                                  pin_memory=True,
                                                  worker_init_fn=my_worker_init_fn,
                                                 )
        if hasattr(self.configs.data, 'val') and self.rank == 0:
            if self.configs.data.val.type == 'tempoSpatial_master_3d':
                ...
                for dataset_type in self.dataset_type_li:
                    self.master_dataloaders['val'][dataset_type] = udata.DataLoader(self.master_datasets['val'][dataset_type],
    ...
    def experiment_evaluate(self):
        # image saving
        if self.rank == 0 and self.configs.experiment_evaluate.save_images:
            image_dir = Path(self.configs.experiment_evaluate.save_path)
            if not image_dir.exists():
                (image_dir / 'expr').mkdir(parents=True)
            self.image_dir = image_dir
        if self.rank == 0:
            self.logger = logger
            self.logger.remove()
            self.logger.add(sys.stdout, format="{message}", level="INFO")
            self.log_step = {phase: 1 for phase in ['expr']}
            self.log_step_img = {phase: 1 for phase in ['expr']}
            self.logger.info(OmegaConf.to_yaml(self.configs))
    ...
    def train(self):
        ...
        for ii in range(self.iters_start, self.configs.train.iterations):
            ...
            # prepare data
            data = self.prepare_data(next(self.dataloaders['train']), realesrgan = True)
            # training phase
            ...
            if 'val' in self.dataloaders and (ii+1) % self.configs.train.get('val_freq', 10000) == 0:
                if self.configs.data.val.type == 'tempoSpatial_master_3d':
                    for dataset_type in self.dataset_type_li:
                        self.datasets['val'] = self.master_datasets['val'][dataset_type]
                        self.dataloaders['val'] = self.master_dataloaders['val'][dataset_type]
                        self.logger.info('validation results for dataset type: {:s}'.format(dataset_type))
                        self.validation(phase = 'val')
                else:
                    self.validation(phase = 'val')
    ...
    def log_step_train(self, loss, tt, batch, z_t, z0_pred, flag=False, phase='train'):
        ...
        if self.rank == 0:
            ...
            if self.current_iters % self.configs.train.log_freq[1] == 0 and flag:
                x1 = vutils.make_grid(batch['lq'][:,3:6, :, :], normalize=True, scale_each=True)  # c x h x w
...
class TrainerDifIR(TrainerBase):
    ...
    def build_model(self):
        ...
        if self.configs.model.ckpt_path is not None:
            ...
            util_net.reload_model(self.model, ckpt, strict = True)
    ...
    def prepare_data(self, data, dtype=torch.float32, realesrgan=None, phase='train'):
        # output should be [-1 to +1]

        num_frames = 3
        if realesrgan is None:
            realesrgan = (self.configs.data.get(phase, dict).type == 'realesrgan') or (self.configs.data.get(phase, dict).type == 'realesrgan_SurgT')
        if realesrgan and phase == 'train':
            # input should be [0 - 1]
            if self.configs.data.train.type == 'realesrgan_master_3d':
                for key in data:
                    data[key] = data[key].squeeze(dim = 1)
            if not hasattr(self, 'jpeger'):
                self.jpeger = DiffJPEG(differentiable=False).cuda()  # simulate JPEG compression artifacts
            if not hasattr(self, 'use_sharpener'):
                self.use_sharpener = USMSharp().cuda()

            im_gt_12ch = data['gt'].cuda() # BxCxHxW with C = 12
            kernel1 = data['kernel1'].cuda()
            kernel2 = data['kernel2'].cuda()
            sinc_kernel = data['sinc_kernel'].cuda()

            ori_h, ori_w = im_gt_12ch.size()[2:4]
            ...
            if self.configs.degradation.use_sharp:
                im_gt_12ch = self.use_sharpener(im_gt_12ch)
            ...
            for i in range(num_frames):
                out[:, i*num_frames: i*num_frames + num_frames, :, :] = filter2D(out[:, i*num_frames: i*num_frames + num_frames, :, :].clone(), kernel1)
            # random resize
            ...
            if random.random() < self.configs.degradation['gaussian_noise_prob']:
                ...
                for i in range(num_frames):
                    out[:, i*num_frames: i*num_frames + num_frames, :, :] = random_add_poisson_noise_pt(
                        out[:, i*num_frames: i*num_frames + num_frames, :, :].clone(),
            ...
            for i in range(num_frames):
                pass ### out[:, i*num_frames: i*num_frames + num_frames, :, :] = self.jpeger(out[:, i*num_frames: i*num_frames + num_frames, :, :].clone(), quality=jpeg_p)
            ...
            if random.random() < self.configs.degradation['second_order_prob']:
                ...
                if random.random() < self.configs.degradation['second_blur_prob']:
                    for i in range(num_frames):
                        out[:, i*num_frames: i*num_frames + num_frames, :, :] = filter2D(out[:, i*num_frames: i*num_frames + num_frames, :, :].clone(), kernel2)
                # random resize
                updown_type = random.choices(
                        ['up', 'down', 'keep'],
            ...
            if random.random() < 0.5:
                ...
                for i in range(num_frames):
                    out[:, i*num_frames: i*num_frames + num_frames, :, :] = filter2D(out[:, i*num_frames: i*num_frames + num_frames, :, :].clone(), sinc_kernel)
                # JPEG compression
            ...
            # clamp and round
            im_lq = torch.clamp((out * 255.0).round(), 0, 255) / 255. # [0 - 1]

            # random crop
            gt_size = self.configs.degradation['gt_size']
            ### assign golden GT to image GT ##
            im_gt = im_gt_golden
            ###################################
        ...
        elif realesrgan and phase == 'expr':
            ...
            out = torch.clamp(out, 0, 1)  # clamp to [0, 1], otherwise JPEGer will result in unpleasant artifacts
            for i in range(num_frames):
                pass ### out[:, i*num_frames: i*num_frames + num_frames, :, :] = self.jpeger(out[:, i*num_frames: (i+1)*num_frames, :, :].clone(), quality=jpeg_p)
        ...
        elif realesrgan == False and phase == 'expr':
            # input should be [0 - 1]
            im_gt_12ch = data['gt'].cuda() # BCHW with C=6
            ori_h, ori_w = im_gt_12ch.size()[2:4]
            sf = self.configs.degradation.sf
            im_gt, im_gt_golden = im_gt_12ch[:,0:9, :, :], im_gt_12ch[:,9:12, :, :]
            im_lq = F.interpolate(
    ...
    def training_step(self, data):
        ...
        for jj in range(0, current_batchsize, micro_batchsize):
            ...
            with torch.no_grad():
                global temp_step
                temp_baseline = vutils.make_grid(micro_data['lq'][0:8, 0:3, :, :].detach().clone(), normalize=True)
                util_image.imwrite(temp_baseline.cpu().permute(1,2,0).numpy(),
                                self.image_dir / 'train' / f"temp_train_{temp_step}_lq0.png",
                                )

                temp_baseline = vutils.make_grid(micro_data['lq'][0:8, 3:6, :, :].detach().clone(), normalize=True)
                util_image.imwrite(temp_baseline.cpu().permute(1,2,0).numpy(),
                                self.image_dir / 'train' / f"temp_train_{temp_step}_lq1.png",
                                )

                temp_baseline = vutils.make_grid(micro_data['lq'][0:8, 6:9, :, :].detach().clone(), normalize=True)
                util_image.imwrite(temp_baseline.cpu().permute(1,2,0).numpy(),
                                self.image_dir / 'train' / f"temp_train_{temp_step}_lq2.png",
                                )

                temp_baseline = vutils.make_grid(micro_data['gt'][0:8].detach().clone(), normalize=True)
                util_image.imwrite(temp_baseline.cpu().permute(1,2,0).numpy(),
                                self.image_dir / 'train' / f"temp_train_{temp_step}_hq.png",
                                )
                temp_step += 1
            ###############################
            '''
    ...
    def validation(self, phase='val'):
        if self.rank == 0:
            ...
            for ii, data in enumerate(self.dataloaders[phase]):
                ...
                is_3d = (f"UNetModelSwin_3d" in self.configs.model.target)
                im_lq_n_ch = im_lq.detach().clone()
                if not is_3d:
                    im_lq_n_ch = im_lq_n_ch[:, 3:6, :, :]
                
                model_kwargs={'lq':im_lq_n_ch,} if self.configs.model.params.cond_lq else None
                tt = torch.tensor(
                    ...
                    if (num_iters + 1) in indices or num_iters + 1 == 1:
                        for key, value in sample.items():
                            if key in ['sample', 'pred_xstart']:
                                if key == 'sample' and isinstance(value, list):
                                    value = value[1]
                            # if key in ['sample']:
                        ...
                        if phase == 'expr':
                            sample_decode['sample'][:, :, :, :] = sample_decode['sample'][:, :, :, :]
                            sample_decode['pred_xstart'][:, :, :, :] = sample_decode['pred_xstart'][:, :, :, :]
                ...
                if 'gt' in data:
                    temp_psnr = util_image.batch_PSNR(
                    ...
                    if self.configs.train.test_baseline:
                        temp_psnr_baseline = util_image.batch_PSNR(
                            im_baseline[:, 3:6, :, :] * 0.5 + 0.5,
                            im_gt * 0.5 + 0.5,
                            ycbcr=False, ###True,
                            )
                        mean_psnr_baseline += temp_psnr_baseline
                        mean_ssim_baseline += util_image.batch_SSIM(
                                im_baseline[:, 3:6, :, :] * 0.5 + 0.5,
                                im_gt * 0.5 + 0.5,
                                ycbcr=False, ###True,
                                )
                        mean_lpips_baseline += self.lpips_loss(im_baseline[:, 3:6, :, :], im_gt).sum().item()

                        if phase == 'expr':
                            temp_psnr /= im_gt.shape[0]; temp_psnr_baseline /= im_gt.shape[0]
                            self.logger.info(f'Teacher expr, psnr: model/baseline: {temp_psnr}/{temp_psnr_baseline}')
                    
                if (ii + 1) % self.configs.train.log_freq[2] == 0:
                    is_cardiac = (phase == 'expr') and (self.configs.data.train.type in ['realesrgan_cardiac_bowl_3d'])
                    if is_cardiac:
                        st_ch_idx, en_ch_idx = 1, 2
                    else:
                        st_ch_idx, en_ch_idx = 0, 3
                    self.logger.info(f'Validation: {ii+1:02d}/{num_iters_epoch:02d}...')

                    im_sr_all = rearrange(im_sr_all, 'b (k c) h w -> (b k) c h w', c=im_gt.shape[1])
                    im_xstart_all = rearrange(im_xstart_all, 'b (k c) h w -> (b k) c h w', c=im_gt.shape[1])
                    ...
                    if 'gt' in data:
                        ...
                        if self.configs.train.save_images:
                            util_image.imwrite(
                                   x4.cpu().permute(1,2,0).numpy()[:, :, st_ch_idx: en_ch_idx],
                                   self.image_dir / phase / f"hq_{self.log_step_img[phase]:05d}.png",
                                   )
                            if self.configs.train.test_baseline:
                                temp_baseline = vutils.make_grid(im_baseline[:, 3:6, :, :], normalize=True)
                                util_image.imwrite(
                                   temp_baseline.cpu().permute(1,2,0).numpy()[:, :, st_ch_idx: en_ch_idx],
                    ...
                    if self.configs.train.save_images and phase == 'expr':
                        ...
                        util_image.imwrite(
                               im_sr_model.cpu().permute(1,2,0).numpy()[:, :, st_ch_idx: en_ch_idx],
    ...
    def training_step(self, data):
        ...
        for jj in range(0, current_batchsize, micro_batchsize):
            ...
            compute_losses = functools.partial(
                ...
                self.model,
                self.teacher_model,
                micro_data['gt'], # [-1 to 1]# image range 0-1
                micro_data['lq'],
                tt,
                first_stage_model=self.autoencoder,
                model_kwargs=model_kwargs,
    ...
    def validation(self, phase='val'):
        ...
        if self.rank == 0:
            ...
            for ii, data in enumerate(self.dataloaders[phase]):
                ...
                self.logger.info(f'latency_list: {latency_list}')
                ...
                if 'gt' in data:
                    ...
                    if self.configs.train.test_baseline:
                        ...
                        if phase == 'expr':
                            temp_psnr /= im_gt.shape[0]; temp_psnr_baseline /= im_gt.shape[0]
                            self.logger.info(f'Distill expr, psnr: model/baseline: {temp_psnr}/{temp_psnr_baseline}')
                ...
                if (ii + 1) % self.configs.train.log_freq[2] == 0:
                    ...
                    if self.configs.train.save_images and phase == 'expr':
                        im_sr_model = vutils.make_grid(results.detach(), normalize=True)
                        util_image.imwrite(
                               im_sr_model.cpu().permute(1,2,0).numpy()[:, :, st_ch_idx: en_ch_idx],
                               self.image_dir / phase / f"sr_{self.log_step_img[phase]:05d}.png",
                               )
...
def my_worker_init_fn(worker_id):
    ...
    #dataset = worker_info.dataset  # The dataset copy owned by this worker
    #for video_id, video_struct in enumerate(dataset.paths):
        #video_struct.cap = cv2.VideoCapture(video_struct.video_path)  # Initialize VideoCapture for each worker
