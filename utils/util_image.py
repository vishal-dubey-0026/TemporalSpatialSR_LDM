...
def batch_PSNR(img, imclean, border=0, ycbcr=False):
    ...
    imclean = imclean.clamp_(0.0, 1.0)
    ...
    Img = (Img * 255).astype(np.uint8) ###img_as_ubyte(Img)
    Iclean = (Iclean * 255).astype(np.uint8) ###img_as_ubyte(Iclean)
    PSNR = 0
    h, w = Iclean.shape[2:]
...
def rgb2ycbcrTorch(im, only_y=True):
    ...
    im_temp = im.permute([0,2,3,1]) * 255.0  # N x C x H x W --> N x H x W x C
    # convert
    if only_y:
    ...
    rlt /= 255.0
    rlt.clamp_(0.0, 1.0)
    return rlt.permute([0, 3, 1, 2]) #N x H x W x C --> N x C x H x W
