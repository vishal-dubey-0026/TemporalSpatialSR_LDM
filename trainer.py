    ...
    def build_dataloader(self):
        ...
        if self.rank == 0:
            dataloaders['expr'] = udata.DataLoader(datasets['expr'],
                                                  ...
                                                  prefetch_factor=self.configs.experiment_evaluate.prefetch_factor,
                                                  pin_memory=True,
                                                 )
        if hasattr(self.configs.data, 'val') and self.rank == 0:
...
class TrainerDifIR(TrainerBase):
    ...
    def prepare_data(self, data, dtype=torch.float32, realesrgan=None, phase='train'):
        ...
        if realesrgan and phase == 'train':
            ...
            if self.configs.degradation.use_sharp:
                im_gt_6ch = self.use_sharpener(im_gt_6ch)
            ...
            # input should be [0 - 1]
            im_gt_6ch = data['gt'].cuda() # BCHW with C=6
            ori_h, ori_w = im_gt_6ch.size()[2:4]
            sf = self.configs.degradation.sf
            im_gt, im_gt_golden = im_gt_6ch[:,0:3, :, :], im_gt_6ch[:,3:6, :, :]
            im_lq = F.interpolate(
    ...
    def training_step(self, data):
        ...
        for jj in range(0, current_batchsize, micro_batchsize):
            ...
            with torch.no_grad():
                global temp_step
                temp_baseline = vutils.make_grid(micro_data['lq'][0:3].detach().clone(), normalize=True)
                util_image.imwrite(temp_baseline.cpu().permute(1,2,0).numpy(),
                                self.image_dir / 'train' / f"temp_train_lq_{temp_step}.png",
                                )

                temp_baseline = vutils.make_grid(micro_data['gt'][0:3].detach().clone(), normalize=True)
                util_image.imwrite(temp_baseline.cpu().permute(1,2,0).numpy(),
                                self.image_dir / 'train' / f"temp_train_hq_{temp_step}.png",
                                )
                temp_step += 1
            ###############################
            '''
            ...
            for ii, data in enumerate(self.dataloaders[phase]):
                ...
                for sample in self.base_diffusion.p_sample_loop_progressive(
                    ...
                    if (num_iters + 1) in indices or num_iters + 1 == 1:
                        ...
                        if phase == 'expr':
                            sample_decode['sample'][:, :, :, :] = sample_decode['sample'][:, 1:2, :, :]
                            sample_decode['pred_xstart'][:, :, :, :] = sample_decode['pred_xstart'][:, 1:2, :, :]
