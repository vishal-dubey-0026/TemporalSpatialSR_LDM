...
from . import gaussian_diffusion_3d as gd_3d
from .respace import SpacedDiffusion, space_timesteps, SpacedDiffusionDDPM, SpacedDiffusion_3d
...
def create_gaussian_diffusion_3d(
    *,
    ...
    sqrt_etas = gd_3d.get_named_eta_schedule(
            schedule_name,
            num_diffusion_timesteps=steps,
            min_noise_level=min_noise_level,
            etas_end=etas_end,
    ...
    if timestep_respacing is None:
        ...
        assert isinstance(timestep_respacing, int)
    if predict_type == 'xstart':
        model_mean_type = gd_3d.ModelMeanType.START_X
    elif predict_type == 'epsilon':
        model_mean_type = gd_3d.ModelMeanType.EPSILON
    elif predict_type == 'epsilon_scale':
        model_mean_type = gd_3d.ModelMeanType.EPSILON_SCALE
    elif predict_type == 'residual':
        model_mean_type = gd_3d.ModelMeanType.RESIDUAL
    else:
        raise ValueError(f'Unknown Predicted type: {predict_type}')
    return SpacedDiffusion_3d(
        use_timesteps=space_timesteps(steps, timestep_respacing),
        sqrt_etas=sqrt_etas,
        kappa=kappa,
        model_mean_type=model_mean_type,
        loss_type=gd_3d.LossType.WEIGHTED_MSE if weighted_mse else gd_3d.LossType.MSE,
        scale_factor=scale_factor,
        normalize_input=normalize_input,
        sf=sf,
        latent_flag=latent_flag,
