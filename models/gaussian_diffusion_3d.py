...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        for i in range(num_frames): 
            x_t_scaled.append(self._scale_input(x_t[i], t))
        model_output = model(x_t_scaled, t, **model_kwargs)


        model_variance = _extract_into_tensor(self.posterior_variance, t, x_t[1].shape)
        model_log_variance = _extract_into_tensor(self.posterior_log_variance_clipped, t, x_t[1].shape)

        # ddim coef
        ddim_coef1 = _extract_into_tensor(self.ddim_coef1, t, x_t[1].shape) # etas_pre*etas
        ddim_coef2 = _extract_into_tensor(self.ddim_coef2, t, x_t[1].shape) # etas_pre/etas
        etas = _extract_into_tensor(self.etas, t, x_t[1].shape)
        etas_prev = _extract_into_tensor(self.etas_prev, t, x_t[1].shape)
        ...
        if self.model_mean_type == ModelMeanType.START_X:      # predict x_0
            ...
            pred_xstart = process_xstart(
                self._predict_xstart_from_residual(y=y[1], residual=model_output)
                )
        elif self.model_mean_type == ModelMeanType.EPSILON:
            pred_xstart = process_xstart(
                self._predict_xstart_from_eps(x_t=x_t[1], y=y[1], t=t, eps=model_output)
            )                                                  #  predict \eps
        elif self.model_mean_type == ModelMeanType.EPSILON_SCALE:
            pred_xstart = process_xstart(
                self._predict_xstart_from_eps_scale(x_t=x_t[1], y=y[1], t=t, eps=model_output)
            )                                                  #  predict \eps
        else:
        ...
        for i in range(num_frames): 
            ...
            model_mean, _, _ = self.q_posterior_mean_variance(
                x_start=pred_xstart, x_t=x_t[i], t=t
            )
            model_mean_li.append(model_mean)

        assert (
            model_mean.shape == model_log_variance.shape == pred_xstart.shape == x_t[1].shape
        )
        return {
            "mean": model_mean_li,
    ...
    def p_sample(self, model, x, y, t, clip_denoised=True, denoised_fn=None, model_kwargs=None, noise_repeat=False):
        ...
        noise = self.custom_randn_like(y[1])
        if noise_repeat:
            noise = noise[0,].repeat(y[1].shape[0], 1, 1, 1)
        
        nonzero_mask = (
            (t != 0).float().view(-1, *([1] * (len(y[1].shape) - 1)))
        )  # no noise when t == 0
        num_frames = len(y)
        sample_li = []
        for i in range(num_frames):
            sample = out["mean"][i] + nonzero_mask * th.exp(0.5 * out["log_variance"]) * noise
            sample_li.append(sample)
        return {"sample": sample_li, "pred_xstart": out["pred_xstart"], "mean":out["mean"]}
    ...
    def p_sample_loop_progressive(
        ...
        z_y = []
        for i in range(num_frames):
            z_y.append(self.encode_first_stage(y[:, i*num_frames: i*num_frames + 3, :, :], first_stage_model, up_sample=True))
        for i in range(num_frames):
            pass ### z_y[i] = z_y[1]
        # generating noise
        if noise is None:
            noise = self.custom_randn_like(z_y[1])
        if noise_repeat:
            noise = noise[0,].repeat(z_y[1].shape[0], 1, 1, 1)
        z_sample = []
        for i in range(num_frames):
            z_sample.append(self.prior_sample(z_y[i], noise = noise))
            '''
            if i == 1:
                z_sample.append(self.prior_sample(z_y[i], noise = None if i != 1 else noise))
            else:
                z_sample.append(z_y[i])
            '''
        ...
        for i in indices:
            ...
            with th.no_grad():
                ...
                if one_step:
                    ...
                    break
                yield out
                z_sample = out["sample"] #z_sample[1] = out["sample"]
    ...
    def training_losses_distill(
        ...
        num_frames = 3
        for i in range(num_frames):
            z_y.append(self.encode_first_stage(y[:, i*num_frames: i*num_frames + 3, :, :], first_stage_model, up_sample=True))  # TODO can be eliminated to speed up, since z_y is already obtained in self.ddim_sample_loop/p_sample_loop
        ...
        z_t = []
        for i in range(num_frames):
            z_t.append(self.prior_sample(z_y[i], noise = noise))
        pred_zstart = None
        ...
        if True:
            ...
            z_start_teacher = z_start_teacher[1]
            ...
            if self.loss_type == LossType.MSE or self.loss_type == LossType.WEIGHTED_MSE:
                for i in range(num_frames): 
                    z_t_scaled.append(self._scale_input(z_t[i], t))
                model_output = model(z_t_scaled, t, **model_kwargs)
                ...
                target = {
                    ...
                    ModelMeanType.RESIDUAL: z_y[1] - z_start,
                ...
                if learn_xT:
                    z_start_teacher_scaled = []
                    for i in range(num_frames):
                        z_start_teacher_scaled.append(self._scale_input(z_start_teacher, t))
                        
                    predicted_xT = model(z_start_teacher_scaled, t*0, **model_kwargs) # TODO scale_input有必要吗？
                    terms[loss_type+"_xT"] = mean_flat((z_t[1] - predicted_xT) ** 2 if loss_type=="mse" else (z_t[1] - predicted_xT).abs())
            ...
            if self.model_mean_type == ModelMeanType.START_X:      # predict x_0
                ...
                pred_zstart = self._predict_xstart_from_eps(x_t=z_t[1], y=z_y[1], t=t, eps=model_output.detach())
            elif self.model_mean_type == ModelMeanType.RESIDUAL:
                pred_zstart = self._predict_xstart_from_residual(y=z_y[1], residual=model_output.detach())
            elif self.model_mean_type == ModelMeanType.EPSILON_SCALE:
                pred_zstart = self._predict_xstart_from_eps_scale(x_t=z_t[1], y=z_y[1], t=t, eps=model_output.detach())
            else:
        ...
        if finetune_use_gt:
            ...
            z_start_gt_scaled = []
            for i in range(num_frames):
                z_start_gt_scaled.append(self._scale_input(z_start_gt, t))

            if not xT_cov_loss:
                with th.no_grad():
                    predicted_xT_from_gt = model(z_start_gt_scaled, t*0, **model_kwargs)
                    
                    xT_std_align = False
                    
                    if xT_std_align:
                        noise_gt_pred = predicted_xT_from_gt-z_y[1]
                        sampled_noise = z_t[1]-z_y[1]
                        noise_gt_new=noise_gt_pred/noise_gt_pred.std()*sampled_noise.std()
                        predicted_xT_from_gt=z_y[1]+noise_gt_new
            else:
                predicted_xT_from_gt = model(z_start_gt_scaled, t*0, **model_kwargs)
                noise_gt_pred = predicted_xT_from_gt-z_y[1].detach()
                terms["mse_xT_cov"] = self.cov_loss(noise_gt_pred)
                terms["loss"] += (terms["mse_xT_cov"]*xT_cov_loss)

            predicted_xT_from_gt_scaled = []
            for i in range(num_frames):
                predicted_xT_from_gt_scaled.append(self._scale_input(predicted_xT_from_gt.detach(), t))
            
            model_output_pedict_gt = model(predicted_xT_from_gt_scaled, t, **model_kwargs)
        ...
        return terms, z_t[1], pred_zstart
    ...
    def training_losses(
        ...
        z_t = []
        for i in range(num_frames):
            z_t.append(self.q_sample(z_start, z_y[i], t, noise = noise))
        #z_t = torch.cat(z_t, dim = 1) # Bx3CxHxW
        terms = {}
        z_t_scaled = []
        if self.loss_type == LossType.MSE or self.loss_type == LossType.WEIGHTED_MSE:
            for i in range(num_frames): 
    ...
    def ddim_sample(
        ...
        return {"sample": sample_li, "pred_xstart": out["pred_xstart"]}
    ...
    def ddim_sample_loop(
        ...
        for sample in self.ddim_sample_loop_progressive(
            ...
            final = sample
        if apply_decoder:
            if isinstance(final["sample"], list):
