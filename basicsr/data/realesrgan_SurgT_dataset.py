...
class RealESRGANSurgTDataset(data.Dataset):
    ...
    def __init__(self, opt, mode='training'):
        super(RealESRGANSurgTDataset, self).__init__()
        self.opt = opt
        ...
        self.MAX_VAL = 255
        self.select_frame_prob = 0.5
        assert self.select_frame_prob <= 0.5, f"select_frame_prob:{self.select_frame_prob} but should be <= 0.5"
        ...
        if 'txt_file_path' in opt:
            for current_txt in opt['txt_file_path']:
                ...
                for ii, case_path in enumerate(videos_path):
                    ...
                    case_path = f'{root_path}/{case_path}'
    ...
    def get_valid_frame_restartstatus(self, video_struct):
        ...
        return (frame, restart_status)

    def __getitem__(self, index):
        if self.file_client is None:
            pass ### self.file_client = FileClient(self.io_backend_opt.pop('type'), **self.io_backend_opt)
        retrial = True
        while retrial:
            retrial = False
            index = random.randint(0, len(self.paths) - 1) #random.sample(population = list(range(len(self.paths))), k = 1)
            video_struct = self.paths[index]
            if video_struct.cap is None:
                video_struct.video_restart()
            temp_frame_count = video_struct.get_total_frames()
            ...
            select_frame_num = int(1 / (self.select_frame_prob)) - 1
            while random.randint(0, select_frame_num) != 0:
                _ = self.get_valid_frame_restartstatus(video_struct)
                
            start_frame, _ = self.get_valid_frame_restartstatus(video_struct)
            
            intermediate_frames_buffer = [None] * temporal_SR # (tempora_SR - 1) intermediate frames + end_frame
            intermediate_frame_count = 0
            intermediate_offset = 1
            while intermediate_frame_count < temporal_SR:
                inter_frame, status = self.get_valid_frame_restartstatus(video_struct)
                if status == True:
                    start_frame = inter_frame
                    intermediate_frame_count = 0
                else:
                    intermediate_frames_buffer[intermediate_frame_count] = inter_frame
                    intermediate_frame_count += 1
            end_frame = intermediate_frames_buffer[-1]
            is_first_split = random.randint(0, 1)
            ...
            start_frame, end_frame = self.scale_img(start_frame), self.scale_img(end_frame)       
            ...
            if not (isinstance(start_frame, np.ndarray) and isinstance(end_frame, np.ndarray)):
                retrial = True; continue
            mid_time_idx = random.randint(start_time_step + 1,  end_time_step- 1) # [1 to 7]
            
            

            if (start_time_step + 1) == mid_time_idx: 
                ...
                #left = start_frame
                mid = intermediate_frames_buffer[mid_time_idx - intermediate_offset]; mid = video_struct.split_frame(mid)[is_first_split]; mid = self.scale_img(mid) 
                #right = intermediate_frames_buffer[mid_time_idx + 1 - intermediate_offset]; right = video_struct.split_frame(right)[is_first_split]; right = self.scale_img(right) 
            elif (end_time_step - 1) == mid_time_idx:
                ...
                #left = intermediate_frames_buffer[mid_time_idx - 1 - intermediate_offset]; left = video_struct.split_frame(left)[is_first_split]; left = self.scale_img(left) 
            ...
            if not (isinstance(mid, np.ndarray) and isinstance(mid, np.ndarray) and isinstance(mid, np.ndarray)):
                retrial = True; continue
        ...
        img_gt = np.concatenate([interp_image, GT_image], axis = -1)  # HxWxC with C = 12
        ...
        img_gt = img2tensor([img_gt], bgr2rgb=False, float32=True)[0] #CxHxW with C = 12 [0 - 1]
