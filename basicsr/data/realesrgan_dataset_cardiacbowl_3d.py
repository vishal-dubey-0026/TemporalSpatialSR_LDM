...
class RealESRGANDataset_3d(data.Dataset):
    ...
    def __init__(self, opt, mode='training'):
        super(RealESRGANDataset_3d, self).__init__()
        self.opt = opt
    ...
    def __getitem__(self, index):
        ...
        while retrial:
            ...
            assert len(start_path) == len(end_path), f"length of start_path: {len(start_path)} and end_path: {len(end_path)} are not equal"
            assert len(start_path) == self.num_slices, f"length of start_path: {len(start_path)} and expectee slices: {self.num_slices} are not equal"
            root_path = self.opt['root_path']
            #start_frame, end_frame = [None] * self.num_slices, [None] * self.num_slices
            interp_image, GT_image = [None] * self.num_slices, [None] * self.num_slices
            for i in range(self.num_slices):
                start_path[i] = f'{root_path}/{start_path[i]}'
                end_path[i] = f'{root_path}/{end_path[i]}'
                start_frame, end_frame = self.read_scale_dcm(start_path[i]), self.read_scale_dcm(end_path[i])
                ...
                if (start_time_step + 1) == mid_frame_idx: 
                    ...
                    left = start_frame
                    mid = self.read_scale_dcm(f'{root_path}/' + video_path[mid_frame_idx].split('\t')[i]) # along time dimension sorta
                    right = self.read_scale_dcm(f'{root_path}/' + video_path[mid_frame_idx + 1].split('\t')[i]) 
                    ...
                    left = self.read_scale_dcm(f'{root_path}/' + video_path[mid_frame_idx - 1].split('\t')[i]) 
                    mid = self.read_scale_dcm(f'{root_path}/' + video_path[mid_frame_idx].split('\t')[i]) 
                    right = end_frame
                else:
